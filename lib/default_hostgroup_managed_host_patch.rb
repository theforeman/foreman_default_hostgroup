  ...
  module ClassMethods
    ...
    def group_matches?(fact)
      fact.each do |fact_name, fact_regex|
        ...
        if Setting[:force_hostgroup_match_all_regex]
          if Regexp.new(fact_regex).match(host_fact_value)
            Rails.logger.info "Value #{host_fact_value} for #{fact_name} matches regex #{fact_regex}"
          else
            Rails.logger.info "Value #{host_fact_value} for #{fact_name} does not match regex #{fact_regex}"
            return false
          end
        else
          if Regexp.new(fact_regex).match(host_fact_value)
            Rails.logger.info "Value #{host_fact_value} for #{fact_name} matches one regex #{fact_regex}"
            return true
          end
        end
      end
    ...
    def find_match(facts_map)
      ...
      Rails.logger.info "No match for facts or hostgroup"
      return false
    end
